...
//
// MACA has no PTX-style acquire/release instructions.  We emulate them with
// plain loads/stores + __threadfence_system() for cross-GPU (MTLink)
// visibility.
//
// Known MACA SDK 4.3.3 compiler bugs to avoid:
...
//   - cooperative_groups::this_grid().sync() → not available; host MACA paths
...
// ---------------------------------------------------------------------------
// Non-coherent loads — MACA has no nc/no_allocate cache hints; use volatile.
// int4 volatile copy not supported by MACA compiler; copy field-by-field.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Non-temporal stores — MACA has no nt/no_allocate hints; plain store.
// int4 volatile assignment not supported by MACA compiler; copy field-by-field.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Named barrier — MACA has no bar.sync.
// On MACA, mc_bar_sync is implemented as __syncthreads() (full CTA barrier).
...
// ---------------------------------------------------------------------------
// Grid sync — not available on MACA.  Host always uses separate kernel
// launches, so SEND and RECV never share a kernel invocation.  No-op.
// ---------------------------------------------------------------------------
__device__ __forceinline__ void mc_grid_sync() {}

// ---------------------------------------------------------------------------
// System-level memory fence: MACA requires explicit __threadfence_system()
...
// before any thread proceeds.  On MACA, __syncthreads() does NOT imply a
...
// ---------------------------------------------------------------------------
// Byte-swap helpers — MACA has no __byte_perm; implement manually.
// ---------------------------------------------------------------------------
